...
func (v *validator) ValidateSignInRequest(req SignInRequest) ValidationErrors {
	...
	if err := validateName(req.FirstName, "firstName"); err != nil {
		errors["firstName"] = err.Error()
	}

	if err := validateName(req.LastName, "lastName"); err != nil {
		errors["lastName"] = err.Error()
	}
...
func validateUserName(name string) error {
	...
	if len(name) > MaxNameLength {
		return fmt.Errorf("userName must be no more than %d characters", MaxNameLength)
	}
